package testpackage;

public class Info {
    final static int A = 0, B = 1, C = 2, D = 3, LONGA = 4, LONGB = 5, LONGC = 6, LONGD = 7;

    final static int TIMEKEEPING = 00, TIMEKEEPINGSET = 01;
    final static int TIMER = 10;
    final static int STOPWATCH = 20;
    final static int ALARM = 30, ALARMSET = 31;
    final static int SCHEDULE = 40, SCHEDULESET = 41;
    final static int WORLDTIME = 50;

    final static int SELECTMODE = 60;
    final static int BEEPALARM = 70, BEEPTIMER = 71;

//    final static int TIME_TYPE_CURRENT_TIME = 0;
//    final static int TIME_TYPE_TIMER = 1;
//    final static int TIME_TYPE_STOPWATCH = 2;
//    final static int TIME_TYPE_CURRENT_TIME = 0;
//    final static int TIME_TYPE_TIMER = 1;
//    final static int TIME_TYPE_STOPWATCH = 2;

    final static int TIME_POINTER_NULL = 0;
    final static int TIME_POINTER_HOUR = 1;
    final static int TIME_POINTER_MINUTE = 2;
    final static int TIME_POINTER_SECOND = 3;
    final static int TIME_POINTER_SCHETYPE = 3;
    final static int TIME_POINTER_YEAR = 4;
    final static int TIME_POINTER_MONTH = 5;
    final static int TIME_POINTER_DAY = 6;

    final static int LIST_POINTER_0 = 0;
    final static int LIST_POINTER_1 = 1;
    final static int LIST_POINTER_2 = 2;
    final static int LIST_POINTER_3 = 3;
    final static int LIST_POINTER_4 = 4;
    final static int LIST_POINTER_5 = 5;

    final static int SCHEDULE_TYPE_CLA = 0;
    final static int SCHEDULE_TYPE_MEE = 1;
    final static int SCHEDULE_TYPE_EVE = 2;
    final static int SCHEDULE_TYPE_ASL = 3;
    final static int SCHEDULE_TYPE_ETC = 4;

    final static int BEEP_TYPE_TIMER = 0;
    final static int BEEP_TYPE_ALARM = 1;

    final static int DAY_OF_MONTH[] = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
}
